...
* Pete Brownlow			1/1/13	Add code to sent hostname to server when getting IP address by DHCP
********************************************************************/
#define __DHCP_C
...
static void _DHCPSend(BYTE messageType, BOOL bRenewing)
{
	...
	UDPPut(DHCP_ROUTER);
	UDPPut(DHCP_DNS);
	// UDPPut(DHCP_HOST_NAME);  // Don't request host name as we are supplying it PNB 1/1/13  
	...
	// Add any new paramter request here.
        ...
        // Send the hostname to the DHCP server - Added 1/1/13  PNB
        // So users can see by name if they don't know the IP address
        // See www.microchip.com/forums/fb.ashx?m=6274

        UDPPut(DHCP_HOST_NAME);  // Parameter code - Option 12 (0x0C)

        // Determine length of hostname - strlen may give wrong answer if it's right padded with spaces

        for (j=0; j<15; j++)
            if ((AppConfig.NetBIOSName[j] == ' ') || (AppConfig.NetBIOSName[j] == 0 ))
                break;
        
        UDPPut(j);                              // Send hostname length
        UDPPutArray(AppConfig.NetBIOSName, j);  // Send the hostname

        // End of addition PNB 1/1/13
