...
void AnnounceIP(void)
{
    ...
    // Get correct length of our name, whether zero terminated or space padded

    for (NameLength=0; NameLength<15; NameLength++)
        if ((AppConfig.NetBIOSName[NameLength] == ' ') || (AppConfig.NetBIOSName[NameLength] == 0 ))
            break;

    NameLength = 16;

	UDPPutArray((BYTE*)AppConfig.NetBIOSName, sizeof(AppConfig.NetBIOSName)-2);
    UDPPut(0);
    ...
    UDPPutString(itoa(eeReadShort(EE_PORTNUM),conStr));


	// Send module name
	UDPPutROMString((ROM BYTE*)" CAN" );
    UDPPutROMString(module_type);

    // Send node number

    nodeNumber = (eeReadShort(EE_NN));
  	UDPPutROMString((ROM BYTE*)" " );
    UDPPutString(ultoa(nodeNumber, conStr));

    // Send parameter block

  	UDPPutROMString((ROM BYTE*)"\r\n" );    // delimiter
    paramPtr = (ROM BYTE *) &params;
    UDPPutROMArray(paramPtr, 32);
...
void DiscoveryTask(void)
{
	static enum {
		...
		DISCOVERY_DISABLED
	} DiscoverySM = DISCOVERY_DISABLED; // Disable response as broadcat is done regularly
